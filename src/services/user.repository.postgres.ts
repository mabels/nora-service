...
export class UserRepositoryPostgres implements UserRepository {
    constructor(
        @Inject(PostgresService) private postgress: PostgresService,
    ) {
    }

    async createUserRecordIfNotExists(uid: string) {
        return this.postgress.query(`INSERT INTO appuser (uid) VALUES ($1) ON CONFLICT DO NOTHING`, uid);
    }
    ...
    async updateUserLinked(uid: string, linked: boolean) {
        return this.postgress.query(`UPDATE appuser SET linked = $1 WHERE uid = $2`, linked, uid);
    }
