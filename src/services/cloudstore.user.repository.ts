...
@Injectable()
export class CloudstoreUserRepository {
    ...
    async createUserRecordIfNotExists(uid: string) {
        await this.cloudstore.query(`INSERT INTO appuser (uid) VALUES ($1) ON CONFLICT DO NOTHING`, uid);
    }

    async getUser(uid: string) {
        const [user] = await this.cloudstore.query<User>(`SELECT * FROM appuser WHERE uid = $1`, uid);
        if (!user) { throw new Error('user does not exist'); }
        return user;
    }

    async isUserLinked(uid: string) {
        const rows = await this.cloudstore.query<User>('select linked from appuser where uid = $1', uid);
        if (rows && rows.length === 1) { return rows[0].linked; }
        return false;
    }

    async updateUserLinked(uid: string, linked: boolean) {
        await this.cloudstore.query(`UPDATE appuser SET linked = $1 WHERE uid = $2`, linked, uid);
    }

    async getRefreshToken(uid: string): Promise<number | null> {
        const rows = await this.cloudstore.query<User>(`SELECT refreshToken FROM appuser WHERE uid = $1`, uid);
        if (rows && rows.length === 1) { return rows[0].refreshtoken; }
        return null;
    }

    async getNodeRedTokenVersion(uid: string): Promise<number> {
        const rows = await this.cloudstore.query<User>(`SELECT noderedversion FROM appuser WHERE uid = $1`, uid);
        if (rows && rows.length === 1) { return rows[0].noderedversion; }
        return 1;
    }

    async incrementNoderedTokenVersion(uid: string): Promise<void> {
        await this.cloudstore.query(`UPDATE appuser SET noderedversion = noderedversion + 1 WHERE uid = $1`, uid);
    }
}
